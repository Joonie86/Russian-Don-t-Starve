...
local spinnerFont = { font = text_font, size = 26 }
...
function SandboxScreen:IntialisePopup(popup, group_id)
	...
	local group = self:AddSpinners(popup, spinners, { -100, 0, 0 })
...
function SandboxScreen:AddSpinners(popup, data, user_offset)
    ...
    master_group:SetHAnchor(ANCHOR_LEFT)  
...
function SandboxScreen:OnModifyTasks()
	...
	local group = self:AddSpinners(popup, spinners, { -150, 200, 0 })	
